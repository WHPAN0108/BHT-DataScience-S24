...
birthwt2  <- birthwt %>% filter(age > 30 & smoke == 1)
...
ggplot(data=birthwt_mod, aes(x=lwt, y=bwt))+geom_point(aes(color=race))+  geom_smooth(method =lm,se = F) +facet_grid( race ~smoke )
